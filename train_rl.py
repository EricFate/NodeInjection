...
if __name__ == '__main__':
    ...
    args.device1 = torch.device('cuda:1' if torch.cuda.is_available() else 'cpu')
    ...
    if args.cuda:
        features = torch.from_numpy(features).to(args.device1).float()
        edge_index = torch.from_numpy(edge_index).to(args.device1).long()
        edge_weight = torch.from_numpy(edge_weight).to(args.device1).float()
        labels = torch.from_numpy(labels).to(args.device1).long()
    ...
    model = SGCNModel(K=2, input_size=100,
                      hidden_size=args.hidden, class_num=18, pre_proj_num=2, after_proj_num=2).to(args.device1)
    ...
    env = GCNEnv(args, model, labels, features.size(0), features=features, edge_index=edge_index,
